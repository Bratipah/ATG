...
export function AnalysisRequest() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (data.status === 'FAILED') {
        ...
        setError(data.error || data.execution?.error || 'Agent failed to generate a proposal')
        return
      }
      ...
      const actionId = data.execution?.actionId
      const hasActionId = typeof actionId === 'string' && actionId.startsWith('0x')

      const mappedProposal = {
        proposalId: hasActionId ? actionId : proposal.reason || `proposal-${Date.now()}`,
        reasoning,
        confidence,
        actions: [
          {
  ...
  const handleExecute = () => {
    if (!lastProposal || !lastProposal.actions.length) {
      setError('No proposal available to execute')
      return
    }
    ...
    if (!lastProposal.proposalId || !lastProposal.proposalId.startsWith('0x')) {
      setError('Execution unavailable: missing on-chain action id')
      return
    }
    ...
    if (!tokenFrom || !tokenTo) {
      ...
      setError('Token address not found for this chain')
      return
    }
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-end gap-4 pt-4">
                   ...
                   <Button 
                    ...
                    disabled={isExecuting || !(lastProposal?.proposalId?.startsWith('0x'))}
